...
async fn run_session(
    ...
    while let Some(msg) = rx.recv().await {
        ...
        // Transcription is agent-driven only: a session's agent calls
        // `stt.invoke` (see agents-js/stt.js). cafe-stt deliberately does not
        // auto-transcribe uploaded binary_refs — that produced a second
        // transcription alongside the agent's invoke.
    }
