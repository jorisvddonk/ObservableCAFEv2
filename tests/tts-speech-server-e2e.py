...
# ///
"""
End-to-end test: TTS via speech-server backend.

Same flow as tts-e2e.py but uses TTS_BACKEND=speech-server and a mock
HTTP server standing in for the real speech-server/proxy.

Usage:
    cargo build --release
    uv run tests/tts-speech-server-e2e.py
"""
...
def main():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        print("=== Starting cafe-tts (speech-server backend) ===", file=sys.stderr)
        ...
        try:
                    ...
                    if msg.get("event") == "chunk":
                        ...
                        if target == binary_ref_id:
                            if ann.get("cafe.binary.read_url") and ann.get("cafe.binary.read_token"):
                                ...
                                print(f"  phase 2/3: read credentials received ({read_url[:50]}...)", file=sys.stderr)
            ...
            try:
                ...
                assert len(audio_data) == audio_byte_size, \
                    f"Downloaded {len(audio_data)} bytes, expected {audio_byte_size}"
    ...
    print("=== ALL TTS SPEECH-SERVER E2E TESTS PASSED ===", file=sys.stderr)
